...
class PATH:
    ...
    def add_output_nodes(self, graph):
        ...
        edge_node_index = max([int(node.split("_")[-1]) for node in graph.nodes if "EdgeNode_" in str(node)], default=13) + 1
        ...
        for node in list(graph.nodes):
            ...
            if FinalLeafNode or SplitLeafNode:  # If ExpressionRoot is not None
                ...
                graph.add_node(new_edge_node_id, ID=new_edge_node_id, literal="O"+str(edge_node_index), BipartitePart="U2")
                ...
                if(FinalLeafNode):
                    OutputLine_Map["O"+str(edge_node_index)] = graph.nodes[node].get('ExpressionRoot')
                elif(SplitLeafNode):
                    OutputLine_Map["O"+str(edge_node_index)] = graph.nodes[node].get('split_id')
        ...
        # graph.add_node(intermediate_node_ID, ID=intermediate_node_ID, literal=intermediate_node_ID, BipartitePart="U1")
        ...
        #     graph.add_edge(outputNode, intermediate_node_ID)

        # # Add the end output node and 'U2' as BipartitePart
        # graph.add_node(end_output_edge_node_id, ID=end_output_edge_node_id, literal="O"+str(edge_node_index), BipartitePart="U2")

        # # connect the intermediate node to the end output node
        # graph.add_edge(intermediate_node_ID, end_output_edge_node_id)
    ...
    def CrossbarDesignRelalization(self):
        ...
        for Processed_graphs_Map_key in self.Processed_graphs_Map:
            ...
            colMap, rowMap, bit_line_counter, word_line_counter = {}, {}, 0, 0
            for node in processed_graph.nodes:
                ...
                if(processed_graph.nodes[node]['BipartitePart']=='U1'):
                    rowMap[processed_graph.nodes[node]['ID']] = word_line_counter
                    word_line_counter += 1

            # print(colMap, rowMap)
            #Setting dimention of BDD
            self.Processed_graphs_Map[Processed_graphs_Map_key]['BDD_dimentions'] = f"{word_line_counter} x {bit_line_counter}"
            ...
            self.Processed_graphs_Map[Processed_graphs_Map_key]['Crossbar_design'] = [[0 for _ in range(bit_line_counter)] for _ in range(word_line_counter)]
