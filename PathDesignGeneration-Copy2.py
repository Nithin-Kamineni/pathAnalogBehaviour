...
class PATH:
    ...
    def get_downstream_subgraph(self, graph, node):
        # Get all descendants (children, grandchildren, etc.)
        descendants = nx.descendants(graph, node)
        
        # Include the starting node itself if needed
    ...
    def split_graphs_with_height(self, graph):
        ...
        top_nodes = [n for n, data in graph.nodes(data=True) if data.get('distance') <= self.HeightThereshold]
        ...
        overflow_split_nodes = sorted([n for n, data in graph.nodes(data=True) if data.get('distance') == self.HeightThereshold and data.get('ExpressionRoot') is None], key=lambda n: graph.nodes[n].get('distance'))

        print('len(overflow_split_nodes)', len(overflow_split_nodes))

        split_graphs = []
        
        for split_node in overflow_split_nodes:
            subgraph = self.get_downstream_subgraph(graph, split_node)
    
            # Generate a unique ID to mark continuity
            split_id = str(uuid.uuid4())
    
            # Add split_id to split_root (leaf in top_graph)
            if split_node in top_graph.nodes:
                print('top_graph.nodes[split_node]',top_graph.nodes[split_node])
                top_graph.nodes[split_node]['split_id'] = f"leaf {split_id}"
    
            # Add split_id to root of subgraph
            print('subgraph.nodes[split_node]',subgraph.nodes[split_node])
            subgraph.nodes[split_node]['split_id'] = f"root {split_id}"
